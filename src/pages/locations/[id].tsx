...
export default function Profile({ data, residentsData }: ProfileProps) {
	...
	return (
		<>
			<div className="p-4">
				<div>
					<Link className="flex hover:cursor-pointer" href={"/locations/"}>
				...
				<section className="residents-container">
					<h2 className="residents-container__title">Residents</h2>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	const res = await fetch(
		`https://rickandmortyapi.com/api/location/${context.query.id}`
	);
	const data = await res.json();

	const residentsQueryIds = data.residents.map((episode: string) =>
		episode.split("/").pop()
	);
