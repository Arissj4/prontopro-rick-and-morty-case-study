...
export default function Locations({ initCharacters, initNextPage }: LocationsProps) {
      ...
      {isLoading && (
			...
			<div className={`page flex flex-col items-center`}>
				...
				<section className="w-full flex flex-col items-center">
					{locations.length > 0 ?
						(locations?.map(location => (
							<SimpleCardComponent type={"location"} data={location} key={location.id} />
						)))
					:
				...
				{locations.length > 0 && nextPage && (
					<Pagination<RMLocation> nextPage={nextPage} setData={setLocations} setNextPage={setNextPage} setIsLoading={setIsLoading}/>
				)}
			</div>
    </>
  )
...
export const getServerSideProps: GetServerSideProps<LocationsProps> = async () => {
