...
export default function Episodes({ initCharacters, initNextPage }: EpisodesProps) {
      ...
      {isLoading && (
			...
			<div className={`page flex flex-col items-center`}>
				...
				<section className="w-full flex flex-col items-center">
					{episodes.length > 0 ?
						(episodes?.map(episode => (
							<SimpleCardComponent type={"episode"} data={episode} key={episode.id} />
						)))
					:
				...
				{episodes.length > 0 && nextPage && (
					<Pagination<RMEpisode> nextPage={nextPage} setData={setLocations} setNextPage={setNextPage} setIsLoading={setIsLoading}/>
				)}
			</div>
    </>
  )
...
export const getServerSideProps: GetServerSideProps<EpisodesProps> = async () => {
