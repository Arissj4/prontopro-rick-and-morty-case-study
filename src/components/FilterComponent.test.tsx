...
describe("Checks Filter component behavior", () => {
  ...
  it("Check rendering the input with costume placeholder", () => {
    ...
    expect(screen.getByPlaceholderText("Jest test placeholder")).toBeInTheDocument();
  });

  it("Calls fetch when user types in input", async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
			json: async () => ({ results: [{id: 1, name: "Rick Sanchez"}], info: { next: "next-page-url" } }),
		});
    ...
    await userEvent.type(screen.getByPlaceholderText("Filter by name..."), "Ri");

    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith("https://rickandmortyapi.com/api/character/?name=ri")
    });
